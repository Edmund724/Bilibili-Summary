// offscreen-asr.js — ASR 音频「下载 → 解码 → 切片 → 转写」全链路（自
...
// entry/offscreen.js，任务终态（断连取消 / done / error）经注入的
...
export function makeAsrSkipError(cause, reason = "") {
...
export function createAsrDecodeHandler({ onTaskTerminal }) {
  return async function handleAsrDecodeTask(task, port) {
    let aborted = false;
...
// （~10 moof / 1MB）可正常解码。
async function resampleTo16kMono(audioCtx, audioBytes) {
