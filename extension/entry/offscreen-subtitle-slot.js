// offscreen-subtitle-slot.js — offscreen 聊天字幕体的「单槽缓存」纯逻辑
// （候选5：追问不重传字幕体）。
//
...
// 视频的字幕体接进当前追问。
//
// 纯逻辑放独立模块：entry/offscreen.js 顶层挂满 chrome 事件监听，不可在
// Node 测试环境导入（与 offscreen-lifecycle.js 同一套拆分手法，可测）。
 ...
 * @param {{ context?: object, contextKey?: string }} msg offscreen-chat 的 chat 消息
 */
export function createSubtitleBodySlot() {
  let slot = null;
  ...
  function settle(msg) {
    ...
    if (context.subtitleBody !== undefined) {
      slot = { contextKey, subtitleBody: context.subtitleBody };
