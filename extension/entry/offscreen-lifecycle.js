// offscreen-lifecycle.js — offscreen 文档自身生命周期的纯判定（可测）。
//
...
//
// 纯函数放独立模块：entry/offscreen.js 顶层挂满 chrome 事件监听，不可在
// Node 测试环境导入。

// currentChatCount 为 0（无存活聊天端口）→ 关；NaN/undefined（计数异常）
// → 不关，保守保留文档。
export function shouldCloseAfterAsrTask(currentChatCount) {
