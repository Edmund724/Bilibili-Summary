// extension/core/host-permissions.js
...
//
// 不变式：
// - baseUrl 必须是 http(s):// 开头的合法 URL（与 validators.js 的
...
export function extractOriginFromBaseUrl(baseUrl) {
...
export function collectOrigins(baseUrls) {
  const origins = [];
...
export async function requestProviderOrigins(baseUrls, requestPermissions = globalThis.chrome?.permissions?.request) {
  ...
  try {
    ...
    return { ok: false, error: `申请域名权限失败：${error?.message || error}`, origins };
  }
  if (!granted) {
    return {
      ok: false,
...
export async function hasHostPermission(target, contains = globalThis.chrome?.permissions?.contains) {
...
export function collectOrphanOrigins(deletedBaseUrl, remainingProviders) {
  const origin = extractOriginFromBaseUrl(deletedBaseUrl);
  if (!origin) {
    return [];
  }
  const remaining = Array.isArray(remainingProviders) ? remainingProviders : [];
  const stillUsed = remaining.some((provider) => extractOriginFromBaseUrl(provider?.baseUrl) === origin);
...
export async function revokeOrphanOrigin(
  ...
  const survivors = (Array.isArray(providers) ? providers : []).filter(
    (provider) => String(provider?.id || "") !== deletedId
  );
...
export const permissionRevokeErrorMessage = (origins) =>
