...
// 参与签名失效判定」又只活在 computeSidepanelStateSignature 的注释里，两处知识
// 分居、改动靠人肉对账。本模块把三件事收进一处：
//   1. 字段清单（SIDEPANEL_CONTEXT_PAYLOAD_FIELDS，含每个字段「给谁消费」的对账注）；
//   2. 组装工厂（createSidepanelContextPayload，message-handler 只喂运行时输入）；
//   3. 签名投影（参与字段集合 + 逐字段投影 + 排除清单，签名从 payload 字段清单
//      派生而非手列 join 数组）。
//
// 契约边界（对账结论，勿在本模块补字段）：SP 实际持有的快照 = 本 payload +
//   { signature }（message-handler 处理器附加）+ { hotComments 覆盖, isVideoContext }
//   （background 的 ai/context-resolver.js getAiSidepanelState 转发层覆盖/补写）。
//   即 isVideoContext 不由 content 组装，SP 侧读取（sidepanel.js）依赖背景层补写。
//
...
//                         resolveAiSidepanelPageRef 兜底。SP UI 无直接读取。
//   subtitleBody          SP 等待轮询（sidepanel-subtitle-wait.js）与无字幕拦截
//                         （sidepanel-no-subtitle.js）的判定数据；chat-runtime 按
...
//   subtitleFetchState    SP 等待轮询（loading）与无字幕拦截（empty）的主信号。
//   noSubtitleReason      SP 发送拦截的按原因提示（sidepanel.js buildNoSubtitleNotice）。
...
//   subtitleOptions       SP 侧当前无直接读取（保留：与 popup 侧 payload 字段同名
...
// 等长新数组视为未变，与 SP 侧 contextKey 的去重语义一致。
//
// 各字段参与理由：
//   bvid / cid             视频身份主键（cid 空回退 aid，无 cid 的老数据/异常页
//                          仍有稳定键）。
//   pageIndex              同视频换分 P 是上下文实质变化。
//   subtitleFetchState     idle/loading/ready/error 覆盖抓取全生命周期，SP 等待
//                          轮询与无字幕拦截都以此为信号。
...
// 签名判定（纯函数，可测）。输入是 createSidepanelContextPayload 产物（即 SP
