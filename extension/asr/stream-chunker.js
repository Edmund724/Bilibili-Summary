...
// 返回 { totalChunks, totalDurationSec, peak }。
export async function streamWavChunks(segments, { chunkSeconds, sampleRate = 16000, decodeSegment, onChunk }) {
  ...
  for await (const segment of segments) {
    const segMono = await decodeSegment(segment);
  ...
  return { totalChunks: emitted, totalDurationSec, peak };
}
