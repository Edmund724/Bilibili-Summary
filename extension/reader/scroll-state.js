// 阅读视图滚动状态微模块。
//
// 这是 SYNC（./sync.js）与 LAYOUT（./reader-impl.js）的共享叶子模块：拥有
// 手动滚动暂停与程序化滚动两个截止时间的唯一声明与读写函数。放在独立叶子
// 里，让 SYNC 与 LAYOUT 共享同一份状态而不需要访问器穿越 reader-impl 的
// 闭包 seam，也保持依赖图无环——本模块不 import reader 域内任何其他模块
// （LAYOUT 仍然不得 import SYNC）。
