...
独立的第二个测试，只回答一个问题：**这个 API 对单账号的并发上限是多少**。做法是逐级加压——对单个模型，用不同并发档位（默认 1 → 2 → 4 → 8 → 16 → 32）各把同一份音频完整转写一遍，观察墙钟在哪一级不再下降。

```bash
npm run eval:sweep
```

- 扫描参数在 `config.json` 的 `sweep` 块：`model`（缺省取 `models` 名单第一个）、`concurrencyLevels`、`runsPerLevel`（默认 1）、`chunkSeconds`（默认 120 秒，切小片测得准）、`audioSeconds`（默认 1800，只取音频前 30 分钟，省流量——每档都要完整转写一遍）。
- 报告写 `eval/out/sweep-report.json` / `sweep-report.md`。
- 判读：并发低于上限时墙钟近似按并发倍数下降；**拐点之后墙钟平台化、段均被服务端排队拉长、可能伴随 429**——首次 429 的档位与"相对上一档加速 < 1.2x"的首档（报告会自动标注）就是有效并发上限的估计。
- 注意成本：每档都会完整上传一遍所用音频（前 30 分钟 ≈ 57MB），档位越多流量越大。
