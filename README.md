...
- 基于当前视频字幕进行多轮问答，也支持在任意网页中作为通用 AI 对话侧边栏使用
- 内置历史对话、预设提示词、模型切换等能力，适合快速总结和提炼视频内容
- 支持自定义系统提示词和开场问题，按需调整 AI 回答风格
- 模型名称输入框旁带有下拉箭头，点击后可一键拉取平台提供的模型列表，无需手动输入模型名称

### 播放器 AI 快捷按钮

在播放器内显示 AI 按钮，支持自定义提示词，点击后直接在侧边栏生成视频摘要；稍后再看页面同样支持。
