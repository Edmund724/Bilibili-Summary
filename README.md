...
- 无字幕时自动尝试语音识别回退
- API Key、笔记、对话与缓存仅存本地；非敏感设置经 Chrome 账号同步。不包含统计追踪或第三方数据中转
...
API Key 保存在你设备上的 Chrome 扩展本地存储中，非敏感设置经 Chrome 账号同步。发布包不会包含或使用 `config.js`。
...
5. API Key、笔记、对话与最近缓存仅存本地（`chrome.storage.local`，不随账号同步）；非敏感设置（系统提示词、下载格式、Frontmatter 字段等）经 `chrome.storage.sync` 随你的 Chrome 账号在登录设备间同步。
