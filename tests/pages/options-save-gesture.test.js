// 侧边栏设置面板保存路径的「用户手势」不变式测试（S2 收紧 host_permissions）。
//
...
// 1. saveSettings 函数体从开头到权限申请调用之间零 await；
// 2. 保存按钮的 click 监听直调 saveSettings，中间不垫 await。
//
// digest-only-ui：原独立 options 页已删除，保存链随设置面板迁移到
// extension/ui/settings-panel.ts（content script 语境无 chrome.permissions
// API，host 权限申请改走 requestProviderOriginsViaBackground → background 的
// request-provider-origins 处理器——手势经一次 runtime 消息传导，SW 处理器在
// 调用 chrome.permissions.request 前零 await，因此本文件不变式原样适用）。
//
...
function readSettingsPanelSource() {
  const relativePath = "../../extension/ui/settings-panel.js";
