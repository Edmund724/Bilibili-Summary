...
// 用户症状（修复前）：视频页首点播放器 AI 键一键总结，对话侧报
// "当前页面上下文读取失败。"——背景上下文链的刷新分支调用 withTimeout，其实现
// 用了 window.setTimeout；SW 全局无 window，Promise 执行器同步抛 ReferenceError
// 使限期竞速立即拒绝，错误经静默发送路径坍缩成通用文案。
